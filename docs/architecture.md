...
5. Chat routes build context from the workspace profile, guardrails, memory, recent messages, documents, and the active Advisor Work Card.
...
Each workspace has one `boardroom_profiles` row. New users complete it before entering Boardroom. Only workspace owners and admins can update it, and the profile remains in place during Fresh Start.
